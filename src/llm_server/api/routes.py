"""Rotas da API para o servidor LLM."""
...
def create_app(server: LlamaServer, health_monitor: ServerHealth) -> Flask:
    """Cria a aplicação Flask com as rotas configuradas."""
    app = Flask(__name__)
    logger = get_logger('llm_server.api')
    ...
    def health():
        ...
        try:
            ...
            monitoring_data = {
                ...
                "uptime_formatted": format_uptime(health_status["uptime_seconds"]),
            ...
            if server.request_count % 20 == 0 and server.request_count > 0:
                logger.info(
                    f"📊 Status de saúde completo (requisição #{server.request_count})"
                )
                logger.info(
                    f"📈 Uptime: {monitoring_data['uptime_formatted']}, Erros: {health_status['consecutive_errors']}, Taxa de erro: {health_status['error_rate_5min']:.2%}"
                )

                # Executar limpeza periódica
                logger.debug("🧹 Executando limpeza de memória periódica...")
                gc.collect()

                # Log métricas no arquivo
                get_logger('llm_metrics').info(
                    f"HEALTH_CHECK: status={health_status['status']}, uptime={health_status['uptime_seconds']:.0f}s, errors={health_status['consecutive_errors']}, error_rate={health_status['error_rate_5min']:.3f}"
                )
